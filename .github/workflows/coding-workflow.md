...
This prompt guides you, a coding agent, to create, debug, update or do other actions related to **GitHub Agentic
Workflows (gh-aw)** in a repository.
...
Follow carefully the instructions in the appropriate prompt file based on the user's request. Read ALL the instructions
in the prompt file before taking any action.

Below, ROOT is the location where you found this file. For example,

- if this file is at `https://raw.githubusercontent.com/github/gh-aw/main/create.md` then the ROOT is
  `https://raw.githubusercontent.com/github/gh-aw/main`
- if this file is at `https://raw.githubusercontent.com/github/gh-aw/v0.35.1/create.md` then the ROOT is
  `https://github.com/github/gh-aw/v0.35.1/main`

Here are the common actions you may be asked to do, with links to the appropriate prompt files:

### Create New Workflow

**Load when**: User wants to create a new workflow from scratch, add automation, or design a workflow that doesn't exist
yet
...
**Load when**: User needs to investigate, audit, debug, or understand a workflow, troubleshoot issues, analyze logs, or
fix errors
...
If you need to clarify requirements or discuss options, and you are working in an interactive agent chat system, do so
interactively with the user. If running non-interactively, make reasonable assumptions based on the repository context.
...
If creating a workflow, the actual files you created will be under `.github/workflows/`. There should be at least one
workflow file and one lock file.
...
Don't remove these but don't add them if not already present in the repo. Unless instructed otherwise do NOT commit the
changes to ANY files except the gitattributes file and workflow files.
...
If there is branch protection on the default branch, create a pull request instead and report the link to the pull
request.
